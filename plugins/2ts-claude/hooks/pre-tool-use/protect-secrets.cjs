...
// Personal default: 'high' protects keys, cloud creds, secrets files, env dumps and
// exfiltration. Bump to 'strict' to also cover db configs, known_hosts, gitconfig.
const SAFETY_LEVEL = 'high';

// Files explicitly safe to access (templates and examples only — never a real .env).
const ALLOWLIST = [
    /\.env\.example$/i,
    /\.env\.sample$/i,
    /\.env\.template$/i,
...
const BASH_PATTERNS = [
    ...
    { level: 'high', id: 'env-dump', regex: /\bprintenv\b|(?:^|[;&|]\s*)env\s*(?:$|[;&|])/, reason: 'Environment dump may expose secrets' },
    {
        level: 'high',
        id: 'echo-secret-var',
        regex: /\becho\b[^;|&]*\$\{?[A-Za-z_]*(?:SECRET|KEY|TOKEN|PASSWORD|PASSW|CREDENTIAL|API_KEY|AUTH|PRIVATE)[A-Za-z_]*\}?/i,
